...
class PasteSketchCommand(TurtleUICommand):
    ...
    def onCreated(self, eventArgs:core.CommandCreatedEventArgs):
        ...
        try:
            ...
            self.guidepoints = []
            self.hasGuidepoint= lambda : len(self.guidepoints) > 0
            self.lastGuidepoint=  lambda : self.guidepoints[-1] if self.hasGuidepoint() else None

            self.guidelines = []
            self.hasGuideline = lambda : len(self.guidelines) > 0
            self.lastGuideline = lambda : self.guidelines[-1] if self.hasGuideline() else None

            gl = TurtleUtils.getSelectedTypeOrNone([f.SketchLine, f.SketchPoint])
            if gl:
                self.addGuideElement(gl)
            else:
            ...
            self.guidelineSelection = inputs.addSelectionInput('selGuideline', 'Select Guide', 'Optional reference guideline/points used if transforming sketch.')
    ...
    def addGuideElement(self, gl):
        if gl and not gl.attributes.itemByName("Turtle", "generated"):
            if isinstance(gl, f.SketchPoint):
                self.guidepoints.append(gl)
            else:
            ...
            if cmdInput.id == 'selGuideline':
                ...
                if self.sketchSelection.selectionCount > 0:
                    ...
                    if(self.hasGuideline and self.guidelines[0].parentSketch != self.sketch):
                        self.guidelines.clear()
                    if(self.hasGuidepoint and self.guidepoints[0].parentSketch != self.sketch):
                        self.guidepoints.clear()
                else:
                    self.sketch = None
                    self.guidelines.clear()
    ...
    def onPreSelect(self, eventArgs:core.SelectionEventArgs):
        ...
        if (type(sel) == f.SketchLine or type(sel) == f.SketchPoint) and sel.attributes.itemByName("Turtle", "generated"):
    ...
    def onExecute(self, eventArgs:core.CommandEventArgs):
        ...
        if self.hasGuideline() and self.data.hasEncodedGuideline:
            self.decoder = TurtleDecoder.createWithGuidelines(self.data, self.guidelines, reverse, mirror)
        elif self.hasGuidepoint() and self.data.hasEncodedGuidepoint:
            pass
            #self.decoder = TurtleDecoder.createWithGuidepoints(self.data, self.guidepoints, reverse, mirror)
        elif self.sketch:
    ...
    def _resetSelections(self):
        ...
        self.guidepoints = [gp for gp in self.guidepoints if not gp.isValid]
        for gp in self.guidepoints:
            self.guidelineSelection.addSelection(gp)
    ...
    def _resetUI(self):
        if self.hasGuideline() or self.hasGuidepoint() or self.isInSketch:
        ...
        if self.hasGuideline() or self.hasGuidepoint():
            self.guidelineSelection.clearSelection() 
            for gl in self.guidelines:
                self.guidelineSelection.addSelection(gl)
            for gp in self.guidepoints:
